...
import { fetchPurchaseOrders } from '../purchaseOrders/purchaseOrderSlice';
...
export const InventoryManagerDashboard: React.FC = () => {
    ...
    }, [productsStatus, categoriesStatus, poStatus, dispatch]);

    const productsByCategory = useMemo(() => {
        if (categoriesStatus !== 'succeeded' || productsStatus !== 'succeeded') return [];
        return categories.map(category => ({
            name: category.name,
            value: products.filter(p => p.categoryId === category.id).length,
        })).filter(c => c.value > 0);
    }, [products, categories, productsStatus, categoriesStatus]);
    ...
    if ([productsStatus, categoriesStatus, poStatus].includes('loading')) {
    ...
    return (
        <div className="p-4 md:p-8 bg-gray-50 min-h-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                ...
                <StatCard title="Product Categories" value={categories.length} icon={TagIcon} />
            </div>
